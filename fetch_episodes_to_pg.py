...
from src.external_api_service import PodscanAPIClient, APIClientError # External podcast search APIs
# Assuming data_processor.parse_date is robust for various date string formats if email.utils.parsedate_to_datetime fails
...
async def process_single_media_episodes(
    ...
    async with sync_semaphore: # Control concurrency for processing each podcast
        ...
        if podscan_api_id and podscan_client:
            ...
            try:
                ...
                podscan_raw_data = await loop.run_in_executor(
                    None, podscan_client.get_podcast_episodes, podscan_api_id, per_page=20 # Fetch a bit more
                )
                if podscan_raw_data:
                    source_used = "Podscan"
        ...
        existing_episode_identifiers: Set[tuple[str, datetime.date]] = await db_service_pg.get_existing_episode_identifiers(media_id)
        ...
        if new_episodes_to_insert_payload:
            ...
            await db_service_pg.insert_episodes_batch(new_episodes_to_insert_payload)
        else:
        ...
        await db_service_pg.trim_old_episodes(media_id, EPISODES_TO_KEEP_PER_PODCAST)
        ...
        await db_service_pg.flag_episodes_for_transcription(media_id, EPISODES_TO_FLAG_FOR_TRANSCRIPTION)
...
async def main_episode_sync_orchestrator():
    ...
    async with aiohttp.ClientSession() as http_session: # Create one session for all RSS fetches
        media_to_sync = await db_service_pg.get_media_to_sync_episodes(interval_hours=DEFAULT_SYNC_INTERVAL_HOURS)
        ...
        if not media_to_sync:
            ...
            tasks = [
                process_single_media_episodes(media_item, http_session, sync_semaphore)
...
if __name__ == "__main__":
    # Ensure environment variables like PODSCANAPI and database connection strings are set
    # For example, load from .env file if you use one:
    # from dotenv import load_dotenv
    # load_dotenv()
    
    asyncio.run(main_episode_sync_orchestrator())
