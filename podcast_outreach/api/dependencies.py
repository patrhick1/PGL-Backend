...
def get_current_user(request: Request) -> Dict[str, Any]:
    ...
    if hasattr(request, 'session'):
        logger.debug(f"request.session type: {type(request.session)}")
        logger.debug(f"request.session content: {dict(request.session) if request.session else 'None or Empty'}")
    else:
    ...
    logger.debug("Session and username key exist. Proceeding to extract user_data.")
