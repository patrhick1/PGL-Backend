...
class VettingAgent:
    ...
    def _extract_client_profile(self, campaign_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract comprehensive client profile from questionnaire responses."""
        ...
        # Extract social enrichment data
    ...
    async def _generate_vetting_checklist(self, client_profile: Dict[str, Any]) -> Optional[VettingChecklist]:
        """Generate comprehensive vetting checklist using all questionnaire data."""
    ...
    async def _gather_podcast_evidence(self, media_id: int) -> str:
        ...
        evidence_parts = [
            ...
            f"Social Followers: Twitter={media_record.get('twitter_followers')}, LinkedIn={media_record.get('linkedin_connections')}",
            f"Quality Score: {media_record.get('quality_score')}",
            f"Publishing Frequency: {media_record.get('publishing_frequency_days')} days",
            f"Total Episodes: {media_record.get('total_episodes')}"
        ]

        # Add compiled episode summaries if available
        if media_record.get('episode_summaries_compiled'):
            evidence_parts.append("\n=== COMPILED EPISODE SUMMARIES ===")
            evidence_parts.append(media_record['episode_summaries_compiled'][:2000])  # Limit to first 2000 chars
        ...
        if episodes:
            ...
            for i, ep in enumerate(episodes[:5]):  # Analyze top 5 episodes
                ...
                evidence_parts.append(f"- Published: {ep.get('publish_date')}")
                ...
                if guest_names:
                    evidence_parts.append(f"- Guest: {guest_names}")
    ...
    async def _score_podcast_against_checklist(self, checklist: VettingChecklist, evidence: str, client_profile: Dict[str, Any]) -> Optional[VettingAnalysis]:
    ...
    async def vet_match(self, campaign_data: Dict[str, Any], media_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            client_profile = self._extract_client_profile(campaign_data)
            ...
            checklist = await self._generate_vetting_checklist(client_profile)
            if not checklist:
                return None
            ...
            analysis = await self._score_podcast_against_checklist(checklist, evidence, client_profile)
            if not analysis:
                return None
